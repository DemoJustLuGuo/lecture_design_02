...
当前实现中，诊断建议模块已经扩展为“规则诊断 + 可选大模型增强”的双层结构。大模型只增强根因说明、关键症状和处理建议，不替代异常检测、故障分类、故障定位和规则库。
...
当前实现中，大模型 API 作为可选增强层接入。系统先生成规则诊断，再在用户主动触发时将结构化故障数据、关键 KPI、定位信息和规则诊断摘要发送给 OpenAI 兼容接口，要求返回结构化 JSON。若未配置 API key、返回非法 JSON、超时或调用失败，系统保留规则诊断结果并提示失败原因。
...
该模块主要输出根因分析、关键症状、推荐处理动作、影响范围说明、是否需要人工复核、复核理由和诊断来源。诊断来源可区分为“规则诊断”和“大模型增强诊断”。
...
| 大模型增强诊断 | 故障、KPI、定位和规则诊断摘要 | OpenAI 兼容 LLM API | 增强诊断接口 | AI 增强诊断按钮和来源标记 |
...
| 设置与数据管理 | `frontend/src/pages/Settings.tsx`, `backend/src/api/routes/simulation.py` |
...
> 模型层与故障检测/分类/定位的实现与评估口径，详见 [`model_layer_and_detection.md`](./model_layer_and_detection.md)。
...
```text
本系统围绕通信网络故障检测与诊断业务流程进行设计，整体划分为数据采集与模拟、故障检测、故障分类、故障定位、诊断建议、日志统计与可视化、前端展示七个功能模块。数据采集与模拟模块负责构建基站、链路指标和故障样本数据；故障检测模块采用监督随机森林为主通道、通信规则与 Isolation Forest 为辅通道的三通道融合判断网络状态是否异常；故障分类模块利用 Random Forest 识别故障类型并输出置信度，对低置信度结果标记人工复核；故障定位模块基于最近多基站采用加权最小二乘三边定位估算故障坐标并计算定位误差；诊断建议模块基于规则库输出根因分析和运维处理建议；日志统计与可视化模块保存故障记录并展示模型性能指标；前端展示模块以管理系统形式展示网络运行状态、故障日志、故障地图、诊断建议和模型评估结果。各模块共同构成从通信网络数据到智能诊断结果展示的完整闭环。
```

如果报告需要突出大模型增强能力，可补充：

```text
在诊断建议模块中，系统进一步引入大模型增强诊断机制。大模型 API 不直接参与异常检测、故障分类和定位计算，而是在规则诊断结果基础上，结合结构化故障类型、置信度、关键 KPI、基站状态和定位误差信息生成更自然、可解释、面向运维人员的根因分析和处理建议。当外部大模型不可用时，系统自动回退到规则诊断结果，保证演示流程稳定。
```
